...
List<ManufacturingOrder> findAllActiveWithAssignee();
    ...
    @Query("""
            SELECT DISTINCT m FROM ManufacturingOrder m
            JOIN FETCH m.finishedProduct
            LEFT JOIN FETCH m.billOfMaterial
            LEFT JOIN FETCH m.assignee
            LEFT JOIN FETCH m.components c
            LEFT JOIN FETCH c.product
            WHERE m.id = :id AND m.active = true
            """)
    Optional<ManufacturingOrder> findActiveWithComponents(Long id);

    @Query("""
            SELECT DISTINCT m FROM ManufacturingOrder m
            LEFT JOIN FETCH m.workOrders w
            LEFT JOIN FETCH w.workCenter
            WHERE m.id = :id AND m.active = true
            """)
    Optional<ManufacturingOrder> findActiveWithWorkOrders(Long id);
